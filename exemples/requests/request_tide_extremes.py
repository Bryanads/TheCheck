...
def fetch_and_save_tide_extremes_data(lat, lon, nome_praia):
    """
    ...
    output_filename = os.path.join(OUTPUT_DIR, f'tide_extremes_data_{nome_praia.replace(" ", "_").lower()}.json')

    print(f"Fetching tide extremes data for {nome_praia} (Lat: {lat}, Lng: {lon})...")
    try:
...
if __name__ == "__main__":
    ...
    fetch_and_save_tide_extremes_data(praia_lat, praia_lon, praia_nome)
