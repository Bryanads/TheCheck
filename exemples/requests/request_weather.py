...
# Parameters for /weather/point endpoint (from your original script)
PARAMS_WEATHER_API = [
...
def fetch_and_save_weather_data(lat, lon, nome_praia):
    """
    ...
    output_filename = os.path.join(OUTPUT_DIR, f'weather_data_{nome_praia.replace(" ", "_").lower()}.json')

    print(f"Fetching weather data for {nome_praia} (Lat: {lat}, Lng: {lon})...")
    try:
...
if __name__ == "__main__":
    ...
    fetch_and_save_weather_data(praia_lat, praia_lon, praia_nome)